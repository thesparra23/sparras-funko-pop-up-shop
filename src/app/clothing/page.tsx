...
export default function ClothingPage() {
  ...
  useEffect(() => {
    const loadProducts = async () => {
      ...
      try {
        // Find the main Clothing category
        const { data: clothingCategory, error: clothingError } =
          await supabase
            .from("categories")
        ...
        if (clothingError) {
          console.error(
            "Error loading Clothing category:",
            clothingError
          );
        }
        ...
        if (clothingCategory) {
          // Find every subcategory underneath Clothing
          const { data: childCategories, error: childError } =
            await supabase
              .from("categories")
              .select("id, name, parent_id")
              .eq("parent_id", clothingCategory.id)
              .order("name", {
                ascending: true,
              });
          ...
          if (childError) {
            console.error(
              "Error loading Clothing subcategories:",
              childError
            );
          }

          clothingCategories = childCategories || [];
        }

        setCategories(clothingCategories);

        // Include Clothing itself as well as every Clothing subcategory.
        const categoryNames = [
          "Clothing",
        ...
        console.error(
          "Unexpected error loading clothing:",
          error
        );
        setProducts([]);
      } finally {
      ...
      <main>
        ...
        >
          ...
          {loading ? (
            ...
            <div>
              {/* Show main Clothing products first */}
              {getProductsForCategory("Clothing").length > 0 && (
                <section
                  style={{
                    marginBottom: "55px",
                  }}
                >
                  ...
                  <div className="figure-grid">
                    {getProductsForCategory("Clothing").map(
                      (product) => (
                        <ProductCard
                          key={product.id}
                          id={product.id}
              ...
              {/* Automatically show every Clothing subcategory */}
